...
.setName('incident_close')
      // TODO: Command Name anpassen
      .setDescription('Schließt einen Incident')
      .addIntegerOption(option => option.setName('incidentid').setDescription('ID des Incidents').setRequired(true))
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    const incidentid = (interaction.options as CommandInteractionOptionResolver).getInteger('incidentid');
    let incident = await db.get(`SELECT * FROM incidents WHERE id = ?`, [incidentid]);
    if (!incident) {
        await interaction.editReply({ content: 'Incident nicht gefunden!' });
        return;
    }
    if (incident.status === "closed") {
        await interaction.editReply({ content: 'Incident ist bereits geschlossen!' });
        return;
    }
    ...
    let embed = new EmbedBuilder()
        ...
        .addFields(
            { name: "Beschreibung", value: incident.description || "Keine Beschreibung angegeben" },
            { name: "Status", value: "Geschlossen" },
            { name: "Dauer", value: `${Math.floor((Date.now() - incident.created_at) / 1000 / 60)} Minuten` },
        )
        .setTimestamp(Date.now());
    await message.edit({ embeds: [embed] });
    await db.run(`UPDATE incidents SET status = "closed" WHERE id = ?`, [incidentid]);
}